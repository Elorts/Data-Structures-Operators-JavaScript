...
/* 
Let's continue with our football betting app! This time, we have a map with a log of the events that happened during the game. The values are 
the events themselves, and the keys are the minutes in which each event happened (a football game has 90 minutes plus some extra time).
...
for (const [key, val] of gameEvents) {
  //if (key < 45) console.log(`[FIRST HALF] ${key}`);
  key < 45
    ? console.log(`[FIRST HALF] ${val}`)
    : console.log(`[SECOND HALF] ${val}`);
}
*/
...
const [t, f, to, ti] = flight.split(";");
console.log(t + " " + f + " " + to + " " + ti);
